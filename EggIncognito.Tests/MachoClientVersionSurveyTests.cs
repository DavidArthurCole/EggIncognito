...
public class MachoClientVersionSurveyTests(ITestOutputHelper output)
{
    ...
    public void Survey_AllHistoricalBinaries()
    {
        ...
        foreach (var appDir in Directory.GetDirectories(Root))
        {
            ...
            catch { output.WriteLine($"{label}: read error"); continue; }
    ...
    private static List<KeyValuePair<int, int>> RankBySites(IReadOnlyList<Arm64Insn> insns, int lo, int hi)
    {
        ...
        for (int i = 0; i < insns.Count; i++)
        {
            ...
            if (!pair.TryGetValue(key, out var set)) pair[key] = set = new();
            set.Add(str.Address);
        }
        ...
        return perValue.OrderByDescending(kv => kv.Value).ToList();
    }
    ...
    private static int? Resolve(IReadOnlyList<Arm64Insn> insns, int strIndex, int reg)
    {
        ...
        for (int j = Math.Max(0, strIndex - 4); j < strIndex; j++)
        {
            ...
            int shift = ins.Rn * 16;
            if (ins.Op == Arm64Op.Movz) { value = (int)(ins.Imm << shift); seeded = true; }
            else if (ins.Op == Arm64Op.Movk && seeded) { int mask = ~(0xFFFF << shift); value = (value & mask) | (int)(ins.Imm << shift); }
        }
    ...
    private static string? ReadShortVersion(string plistPath)
    {
        ...
        try
        {
            ...
            var m = Regex.Match(text, @"CFBundleShortVersionString</key>\s*<string>([^<]+)</string>");
