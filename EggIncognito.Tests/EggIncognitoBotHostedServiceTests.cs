...
public class EggIncognitoBotHostedServiceTests
{
    ...
    public async Task StartAsync_EmptyToken_DoesNotThrow()
    {
        ...
        var holder = new BotInstanceHolder();
        var svc = new EggIncognitoBotHostedService(cfg, NullLogger<EggIncognitoBotHostedService>.Instance, holder);

        await svc.StartAsync(CancellationToken.None);
        await svc.StopAsync(CancellationToken.None);

        Assert.Null(holder.Bot);
    }
}
