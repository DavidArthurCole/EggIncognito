...
public class EiAfxDataBuilderTests
{
    private static string Read(params string[] parts)
    {
        ...
        return File.ReadAllText(Path.Combine(new[] { root, "Endpoints", "default" }.Concat(parts).ToArray()));
    }

    private static EiAfxData Build()
    {
        var icons = DlcArtifactIcons.FromConfigJson(Read("ei", "get_config.json"));
        return EiAfxDataBuilder.BuildFromJson(Read("ei_afx", "config.json"), icons, "test");
    }
