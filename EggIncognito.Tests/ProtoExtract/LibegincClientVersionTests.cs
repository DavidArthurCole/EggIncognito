...
public class LibegincClientVersionTests
{
    ...
    private static byte[] SoWithText(byte[] textBytes)
    {
        ...
        buf[0] = 0x7F; buf[1] = (byte)'E'; buf[2] = (byte)'L'; buf[3] = (byte)'F'; buf[4] = 2; buf[5] = 1;
        ...
        U64(0x28, (ulong)shoff); U16(0x3A, (ushort)shentsize); U16(0x3C, (ushort)shnum); U16(0x3E, (ushort)shstrndx);
        System.Array.Copy(shstrtab, 0, buf, shstrtabOff, shstrtab.Length);
        System.Array.Copy(textBytes, 0, buf, textOff, textBytes.Length);
        int s1 = shoff + shentsize;
        U32(s1 + 0x00, 1); U64(s1 + 0x10, 0x1000); U64(s1 + 0x18, (ulong)textOff); U64(s1 + 0x20, (ulong)textBytes.Length);
        int s2 = shoff + 2 * shentsize;
        U32(s2 + 0x00, 7); U64(s2 + 0x18, (ulong)shstrtabOff); U64(s2 + 0x20, (ulong)shstrtab.Length);
        return buf;
    }
    ...
    private static byte[] Text()
    {
        ...
        for (int i = 0; i < w.Count; i++) for (int k = 0; k < 4; k++) b[i * 4 + k] = (byte)(w[i] >> (8 * k));
        return b;
    }
    ...
    public void Read_ApkZip_PullsSoEntry()
    {
        ...
        Assert.Equal(72, LibegincClientVersion.Read(ms.ToArray(), prevClientVersion: 71));
    }
}
