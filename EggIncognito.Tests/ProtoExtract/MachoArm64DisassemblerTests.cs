...
public class MachoArm64DisassemblerTests {
    ...
    private static uint Add(int rd, int rn, uint imm) => 0x91000000u | ((imm & 0xFFF) << 10) | ((uint)(rn & 0x1F) << 5) | (uint)(rd & 0x1F);
    private static uint LdrS(int rt, int rn, uint imm) => 0xBD400000u | (((imm / 4) & 0xFFF) << 10) | ((uint)(rn & 0x1F) << 5) | (uint)(rt & 0x1F);
    private static uint LdrD(int rt, int rn, uint imm) => 0xFD400000u | (((imm / 8) & 0xFFF) << 10) | ((uint)(rn & 0x1F) << 5) | (uint)(rt & 0x1F);
    private static uint Bl(ulong pc, ulong target) => 0x94000000u | (uint)((((long)target - (long)pc) >> 2) & 0x03FFFFFF);
}
