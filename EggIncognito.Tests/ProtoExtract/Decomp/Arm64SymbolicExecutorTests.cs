...
public class Arm64SymbolicExecutorTests {
    private static uint Fmul(int d, int n, int m) => 0x1E200800u | ((uint)(m & 31) << 16) | ((uint)(n & 31) << 5) | (uint)(d & 31);
    private static uint Fadd(int d, int n, int m) => 0x1E202800u | ((uint)(m & 31) << 16) | ((uint)(n & 31) << 5) | (uint)(d & 31);
    ...
    public void UnknownCall_BecomesOpaque() {
        ...
        var res = RunChain(code, [], (_, _) => null, "s0", out var opaque);
    ...
    private static uint StrS(int t, int n, uint imm) => 0xBD000000u | (((imm / 4) & 0xFFF) << 10) | ((uint)(n & 31) << 5) | (uint)(t & 31);
    ...
    public void KnownCall_AppliesModel() {
        ...
        static ExprNode? resolve(string _, ExprNode[] args) => new Unary(UnOp.Sin, args.Length > 0 ? args[0] : new Const(0));
