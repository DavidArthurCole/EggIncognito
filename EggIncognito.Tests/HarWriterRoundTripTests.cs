...
// re-read by EndpointExtractor.RunFromHar produces the SAME fixture as feeding that flow straight
...
""";
    ...
    {
        var flow = new Cap::CapturedFlow(Url, "POST", 200, RequestDataB64: null, ResponseBodyB64: ResponseB64());
        ...
        direct.Save();
        var directFixture = File.ReadAllText(FixturePath(directRoot));

        // Via HAR: HarWriter emits the flow, RunFromHar reads it back.
        var harRoot = MakeRepo();
        var writer = new Cap::HarWriter();
        writer.Add(flow);
        ...
        Assert.Equal(directFixture, harFixture);
    ...
    {
        var flow = new Cap::CapturedFlow(Url, "POST", 200, RequestDataB64: "AAEC", ResponseBodyB64: ResponseB64());
        var writer = new Cap::HarWriter();
        writer.Add(flow);
