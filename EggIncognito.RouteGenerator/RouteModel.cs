...
public static class RouteParser {
    public static List<RouteModel> Parse(string yaml) {
        ...
        foreach (string rawLine in yaml.Split('\n')) {
            ...
            if (trimmed.StartsWith("- path:", StringComparison.Ordinal)) {
                Flush();
                string path = trimmed.Substring("- path:".Length).Trim().TrimEnd('/');
    ...
    private static void ApplyLine(Block b, string trimmed) {
        if (trimmed.StartsWith("requestType:", StringComparison.Ordinal)) {
            b.LegacyReq = After(trimmed, "requestType:");
        } else if (trimmed.StartsWith("responseType:", StringComparison.Ordinal)) {
            b.LegacyRes = After(trimmed, "responseType:");
        } else if (trimmed.StartsWith("request:", StringComparison.Ordinal)) {
            b.Request = NullIfEmpty(After(trimmed, "request:"));
            b.HasRequest = true;
        } else if (trimmed.StartsWith("response:", StringComparison.Ordinal)) {
            b.Response = NullIfEmpty(After(trimmed, "response:"));
            b.HasResponse = true;
        } else if (trimmed.StartsWith("requestWrapped:", StringComparison.Ordinal)) {
            b.RequestWrapped = After(trimmed, "requestWrapped:") == "true";
        } else if (trimmed.StartsWith("responseWrapped:", StringComparison.Ordinal)) {
            b.ResponseWrapped = After(trimmed, "responseWrapped:") == "true";
        } else if (trimmed.StartsWith("rawResponse:", StringComparison.Ordinal)) {
            b.RawResponse = After(trimmed, "rawResponse:").Trim('"');
        } else if (trimmed.StartsWith("pathParamOnly:", StringComparison.Ordinal)) {
            b.PathParamOnly = After(trimmed, "pathParamOnly:") == "true";
        } else if (trimmed.StartsWith("pathParam:", StringComparison.Ordinal)) {
