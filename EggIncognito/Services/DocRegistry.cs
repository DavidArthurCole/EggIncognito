...
public sealed class DocRegistry : IDocRegistry {
    private static readonly ConfigOption[] ConfigOptions = [
        ...
        new("Identity:ApiUrl", "unset",
            "Internal SyncKit Identity API base url; login wires when ApiUrl + ApiSecret are both set", "auth"),
        new("Identity:ApiSecret", "unset", "Bearer secret for the SyncKit Identity API", "auth"),
        new("Identity:WidgetUrl", "unset",
            "Public SyncKit Identity host url; required for the native login buttons (/auth/sources)", "auth"),
