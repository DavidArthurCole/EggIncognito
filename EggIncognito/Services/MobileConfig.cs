...
public static class MobileConfig {
    public static byte[] BuildCaProfile(byte[] cerDer, string stableId) {
        byte[] idSeed = Encoding.UTF8.GetBytes(stableId);
        string profileUuid = DeterministicUuid(idSeed, "profile");
        string certUuid = DeterministicUuid(idSeed, "cert");
        string idSuffix = DeterministicUuid(idSeed, "ident").ToLowerInvariant();
        string certB64 = Convert.ToBase64String(cerDer);
