...
public sealed class IosStoreChecker(
    ...
    ILogger<IosStoreChecker> logger) : IDeviceStoreChecker {
    ...
    public async Task<StoreCheckResult> CheckAndUpdateAsync(
        ...
        string? host = s["SshHost"];
        string port = s["SshPort"] ?? "2222";
        string? key = s["SshKeyPath"];
        string triggerPath = s["TriggerPath"] ?? "/var/mobile/eggupdate.trigger";
        int pollSeconds = s.GetValue("PollSeconds", 15);
        int pollAttempts = s.GetValue("PollAttempts", 24);
        ...
        [
            ...
            $"root@{host}", "killall -9 AppStore 2>/dev/null || true"
        ], ct);
        try {
        ...
        [
            "-p", port, "-i", key, "-o", "StrictHostKeyChecking=no", "-o", "BatchMode=yes",
            $"root@{host}", "uiopen itms-apps://itunes.apple.com/app/id993492744 || true"
        ], ct);
        try {
        ...
        [
            "-p", port, "-i", key, "-o", "StrictHostKeyChecking=no", "-o", "BatchMode=yes",
            $"root@{host}", $"touch {triggerPath}"
        ], ct);
        if (fire.ExitCode != 0) {
            string note = $"trigger ssh failed: {DeviceParsing.TrimNote(fire.Stderr + fire.Stdout)}";
