...
public sealed class TreeNode {
    ...
    public static TreeNode? Parse(string? jsonStr) {
        ...
        JsonNode? root;
        try { root = JsonNode.Parse(jsonStr); } catch { return null; }
        return Build(null, root, 0, null);
    }

    private static TreeNode Build(string? keyText, JsonNode? value, int depth, string? keyName) {
        ...
        if (value is JsonArray arr) {
            ...
            for (var i = 0; i < arr.Count; i++)
                node.Children.Add(Build(i.ToString(System.Globalization.CultureInfo.InvariantCulture), arr[i], depth + 1, keyName));
    ...
    private static string ValueKind(JsonNode? v) {
        ...
        return jv.TryGetValue(out string? _) ? "string" : "number";
    }
    ...
    private static string LeafValue(JsonNode? v, string kind) {
        ...
        if (kind == "string" && jv.TryGetValue(out string? s) && s is not null)
            return "\"" + s + "\"";
        return kind == "boolean" && jv.TryGetValue(out bool b) ? b ? "true" : "false" : jv.ToJsonString();
    }
}
