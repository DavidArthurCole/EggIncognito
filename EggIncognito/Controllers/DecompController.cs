...
public sealed class DecompController(
    ...
    public async Task<IActionResult> FunctionConstants([FromQuery] string name, [FromQuery] string? device, CancellationToken ct) {
        if (!currentUser.IsAtLeast(SyncKit.Contract.UserRole.Admin))
            return StatusCode(403, new { error = "admin role required" });
        return string.IsNullOrWhiteSpace(name) ? BadRequest(new { error = "name required" }) : await ExtractAsync([name], device, ct);
    }
    ...
    public async Task<IActionResult> GalaxyParticle([FromQuery] string? device, CancellationToken ct) {
        ...
        } catch (DllNotFoundException) { return Ok(new { ok = false, diagnostics = "arm64 disassembler native lib unavailable" }); } catch (Exception ex) { return Ok(new { ok = false, diagnostics = ex.Message }); }
    }
    ...
    public async Task<IActionResult> ResolveVa(
        ...
        try {
            ...
            return embedded.Count > 0
                ? VaResult(tgtBytes, report, embedded[0].Name, embedded[0].Value, textVm, textOff, "contains-fallback", null)
                : Ok(new { ok = false, tier = report.Tier, recovered = report.Recovered, diagnostics = $"{name} not recovered and no referrer found" });
        } catch (DllNotFoundException) { return Ok(new { ok = false, diagnostics = "arm64 disassembler native lib unavailable" }); } catch (Exception ex) { return Ok(new { ok = false, diagnostics = ex.Message }); }
    }
    ...
    public async Task<IActionResult> BuildingEffects([FromQuery] string stem, [FromQuery] string? device, CancellationToken ct) {
        ...
        } catch (System.DllNotFoundException) { return Ok(new { stem, effects = Array.Empty<object>() }); } catch (Exception ex) { return Ok(new { stem, effects = Array.Empty<object>(), diagnostics = ex.Message }); }
    }
    ...
    public async Task<IActionResult> HatcheryAssembly([FromQuery] string? device, CancellationToken ct) {
        ...
        try {
            ...
            return Content(json.ToJsonString(), "application/json");
        } catch (System.DllNotFoundException) { return Ok(new { ok = false, diagnostics = "arm64 disassembler native lib unavailable" }); } catch (System.Exception ex) { return Ok(new { ok = false, diagnostics = ex.Message }); }
    }
    ...
    private static bool TryParseVa(string s, out ulong va) {
        ...
        return s.StartsWith("0x", StringComparison.OrdinalIgnoreCase)
            ? ulong.TryParse(s.AsSpan(2), System.Globalization.NumberStyles.HexNumber, null, out va)
