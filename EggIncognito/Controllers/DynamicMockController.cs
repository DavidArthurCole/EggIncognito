...
// Catch-all for POST paths that no generated controller claimed: DB-only routes added at runtime
// (source = "db" in stored_routes). Attribute routing ranks the generated controllers' concrete
// templates above this {**slug} catch-all, so yaml routes never reach here. POST-only, so it never
// shadows SimulationController's OPTIONS catch-all. Resolves the response proto type from the merged
// route catalog, materializes it from the endpoint store, and returns the same base64 text framing
// the generated controllers use.
[ApiController]
