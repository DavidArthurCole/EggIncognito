...
public class ThemeCssSanitizerTests {
    ...
    public void HostileInput_IsRejectedWhole(string input) {
        var result = ThemeCssParser.Parse(input);
    ...
    public void OutOfRangeValues_AreClamped(string input, string expectedFragment) {
        var result = ThemeCssParser.Parse(FullGroupWrap(input));
    ...
    public void UppercaseInput_IsCaseFolded() {
        var result = ThemeCssParser.Parse("PANEL { COLOR: RED }");
    ...
    public void SettableVar_IsCanonicalized() {
        var result = ThemeCssParser.Parse("panel { color: var(--color-accent) }");
        Assert.True(result.Ok);
    ...
    public void CommentsAreDropped_NeverEmitted() {
        var result = ThemeCssParser.Parse("panel { /* a comment */ color: red }");
        Assert.True(result.Ok);
    ...
    public void TwoShadows_Parse_AndThreeDoNot() {
        Assert.True(ThemeCssParser.Parse("panel { box-shadow: 0 0 2px red, inset 0 1px 4px 2px #001122 }").Ok);
        Assert.False(ThemeCssParser.Parse("panel { box-shadow: 0 0 2px red, 0 0 2px blue, 0 0 2px green }").Ok);
    }

    [Fact]
    public void Gradients_ParseWithinTheGrammar() {
        Assert.True(ThemeCssParser.Parse(
        ...
        Assert.False(ThemeCssParser.Parse(
    ...
    public void EveryCatalogGroup_EnforcesItsPropertyFloor() {
        ...
        Assert.True(ThemeCssParser.Parse("button { opacity: 0.5 }").Ok);
    }
