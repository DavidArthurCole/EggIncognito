...
public class WireBodyTests {
    ...
    public void Normalize_Base64TextBody_UsedAsIs() {
        string b64 = AuthMessageB64();
        (string result, string shape) = WireBody.Normalize(Encoding.ASCII.GetBytes(b64));
    ...
    public void Normalize_RawProtoBytes_Base64Encoded() {
        ...
        (string result, string shape) = WireBody.Normalize(raw);
    ...
    public void Normalize_PlainTextAck_Base64EncodedNotPassedThrough() {
        (string result, string shape) = WireBody.Normalize(Encoding.ASCII.GetBytes("SUCCESS"));
    ...
    public void Normalize_GzippedBase64Text_GunzipsThenUsesText() {
        string inner = AuthMessageB64();
        byte[] gz = Gzip(Encoding.ASCII.GetBytes(inner));
        (string result, string shape) = WireBody.Normalize(gz);
