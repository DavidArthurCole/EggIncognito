...
public static class SampleRpo {
    ...
    public static byte[] Build() {
        ...
        void W32(uint v) {
            BinaryPrimitives.WriteUInt32LittleEndian(u32, v);
            ms.Write(u32);
        }

        void WF(float v) {
            BinaryPrimitives.WriteSingleLittleEndian(u32, v);
            ms.Write(u32);
        }
        ...
        for (int v = 0; v < Positions.Length; v++) {
            foreach (float f in Positions[v]) WF(f);
            foreach (float f in Colors[v]) WF(f);
            foreach (float f in Normals[v]) WF(f);
        }

        foreach (ushort i in Indices) {
            BinaryPrimitives.WriteUInt16LittleEndian(u32, i);
            ms.Write(u32.AsSpan(0, 2));
        }
