...
public class MachoArm64DisassemblerTests {
    ...
    public void Analyze_ResolvesAdrpAddLdr_ToFloatConstants_AndBl() {
        ...
        byte[] code = [.. words.SelectMany(BitConverter.GetBytes)];
        code.CopyTo(bin, 0);

        var res = MachoArm64Disassembler.Analyze(bin, codeVa, codeVa + (ulong)code.Length, codeVa, 0);
    ...
    public void Analyze_ReadsVectorImmediates_FmovBroadcast_AndMoviZero() {
        ...
        uint[] words = [0x4F03F600u, 0x4F000400u];
        byte[] code = [.. words.SelectMany(BitConverter.GetBytes)];

        var res = MachoArm64Disassembler.Analyze(code, codeVa, codeVa + (ulong)code.Length, codeVa, 0);
    ...
    private static uint Add(int rd, int rn, uint imm) =>
        0x91000000u | ((imm & 0xFFF) << 10) | ((uint)(rn & 0x1F) << 5) | (uint)(rd & 0x1F);

    private static uint LdrS(int rt, int rn, uint imm) =>
        0xBD400000u | (((imm / 4) & 0xFFF) << 10) | ((uint)(rn & 0x1F) << 5) | (uint)(rt & 0x1F);

    private static uint LdrD(int rt, int rn, uint imm) =>
        0xFD400000u | (((imm / 8) & 0xFFF) << 10) | ((uint)(rn & 0x1F) << 5) | (uint)(rt & 0x1F);

    private static uint Bl(ulong pc, ulong target) =>
        0x94000000u | (uint)((((long)target - (long)pc) >> 2) & 0x03FFFFFF);
}
