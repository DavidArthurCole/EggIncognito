...
// Os and GameVersion are derived from the session's most-recently-seen decrypted request's rinfo
// (the flow path sees loopback, not the device IP), so they are only surfaced when a single device
// is connected. Os is a human OS name ("iOS"/"Android"); GameVersion is the Egg, Inc. client
// version string (e.g. "1.35.6").
...
public sealed record CaptureStats(
    ...
    // running state immediately (no race with the one-shot /status poll).
    bool Running = false,
    int Port = 0);
