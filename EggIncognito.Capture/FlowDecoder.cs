...
public sealed class FlowDecoder(string contentRoot) {
    private readonly IReadOnlyDictionary<string, string> _responseTypes = EndpointExtractor.LoadEndpointTypes(contentRoot);
    private readonly IReadOnlyDictionary<string, string> _requestTypes = EndpointExtractor.LoadRequestTypes(contentRoot);
    ...
    public DecodeResult DecodeResponse(string path, string responseB64) {
        byte[] respBytes;
        try { respBytes = ProtoFraming.FromBase64Loose(responseB64); } catch { return new(null, null, null, false); }
        ...
        return isAck ? new(null, null, "acknowledgement", Known: true, Ack: true) : new(null, null, null, false);
    }
    ...
    private static string? AsPrintableText(byte[] bytes) {
        ...
        try { return System.Text.Encoding.UTF8.GetString(bytes).Trim(); } catch { return null; }
    }
