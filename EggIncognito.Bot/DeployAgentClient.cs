...
public sealed class DeployAgentClient(string url, string secret) {
    ...
    public async Task<DeployResult> DeployAsync(CancellationToken ct = default) {
        ...
        } catch (TaskCanceledException) { return DeployResult.Failure("Deploy agent timed out."); } catch (HttpRequestException ex) { return DeployResult.Failure($"Could not reach deploy agent: {ex.Message}"); }
    }
