...
public static class CommandParsing {
    ...
    public static ProtoArgs ParseProto(string? subcommand, IReadOnlyList<(string Name, object? Value)> options) => subcommand switch {
        "list" => ProtoArgs.ListPage(
            options.FirstOrDefault(o => o.Name == "page").Value switch { long l => (int)l, int i => i, _ => 1 }),
        "type" => options.FirstOrDefault(o => o.Name == "name").Value is string name && !string.IsNullOrWhiteSpace(name)
