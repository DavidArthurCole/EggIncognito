...
- `KnownVersion` is a metadata-only version list read by the device farm's `StoreAheadCheck` (is the device's store ahead of the installed build?). The automated external-source populators (Fandom/elgranjero/app-store scrapers) were removed; it is written by `KnownVersionRecorder` from iTunes lookup results (`IosStoreCatalog`) and successful on-device update climbs.
