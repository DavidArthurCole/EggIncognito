...
- The capture-to-endpoint pipeline: decode, auto-detect type, redact, self-repair the route map, write the endpoint. Live capture and HAR import funnel through the same per-flow contract, so they cannot diverge.
