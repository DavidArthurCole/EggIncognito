...
// SourceRevisionId MSBuild target). Backs the Discord /verify command. Sha is "unknown" for a
// non-git build (no +suffix).
public sealed record BuildInfo(string Version, string Sha, string ShortSha, string BuildDate, string RepoUrl)
{
    ...
    public static BuildInfo FromAssembly(string repoUrl)
    {
        ...
        // Link-time build date: the assembly file's last-write time, best-effort.
        string buildDate;
