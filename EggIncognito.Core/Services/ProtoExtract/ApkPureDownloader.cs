...
public sealed class ApkPureDownloader(HttpClient http) {
    ...
    public async Task<byte[]?> DownloadApkAsync(string appVersion, CancellationToken ct = default) {
        try {
            string url = $"{Base}/egg-inc/{Package}/download/{Uri.EscapeDataString(appVersion)}";
            var res = await http.GetAsync(url, ct);
            return !res.IsSuccessStatusCode ? null : await res.Content.ReadAsByteArrayAsync(ct);
        } catch {
            return null;
        }
    }
    ...
    public static byte[]? ExtractArmSplit(byte[] downloaded) {
        ...
        try {
            using var zip = new ZipArchive(new MemoryStream(downloaded, false), ZipArchiveMode.Read);
