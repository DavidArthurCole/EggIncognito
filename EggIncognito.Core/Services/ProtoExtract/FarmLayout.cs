...
public static class FarmLayout {
    ...
    private static readonly string[] DefaultHabRow =
        ["hab_chicken_universe", "hab_chicken_universe", "hab_portal", "hab_monolith"];
    ...
    public static IReadOnlyList<Placed> Standard(string defaultHab = DefaultHabPlaceholder) {
        ...
        p.AddRange(ZoneLayout.Resolve(DefaultLab, DefaultHoa, DefaultHatchery, DefaultMissionControl, DefaultFuel,
            DefaultDepot));


        string[] habRow = defaultHab == DefaultHabPlaceholder
            ? DefaultHabRow
            : [defaultHab, defaultHab, defaultHab, defaultHab];
        for (int i = 0; i < 4; i++) {
            float x = (i - 1.5f) * HabSpacing;
    ...
    public static IReadOnlyList<Placed> StandardRecovered(string defaultHab = DefaultHabPlaceholder) {
        ...
        list.AddRange(ZoneLayout.Resolve(DefaultLab, DefaultHoa, DefaultHatchery, DefaultMissionControl, DefaultFuel,
            DefaultDepot));
        return list;
    }

    private static bool IsCoreZoneStem(string s) =>
        s.StartsWith("ei_lab", StringComparison.Ordinal) ||
        s.StartsWith("ei_afx_construction", StringComparison.Ordinal) ||
        s.StartsWith("ei_hoa", StringComparison.Ordinal)
        || s.StartsWith("ei_hatchery", StringComparison.Ordinal) ||
        s.StartsWith("ei_mission_control", StringComparison.Ordinal) ||
        s.StartsWith("ei_fuel_tank", StringComparison.Ordinal)
