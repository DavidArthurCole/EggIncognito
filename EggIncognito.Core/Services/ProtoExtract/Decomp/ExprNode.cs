...
public abstract record ExprNode {
    public static ExprNode Fold(ExprNode n) => n switch {
        ...
        SelectExpr s => new SelectExpr(Fold(s.Cond), Fold(s.A), Fold(s.B)),
    ...
    private static ExprNode FoldUnary(UnOp op, ExprNode x) => x is ConstExpr c
        ? new ConstExpr(op switch {
            UnOp.Neg => -c.V,
    ...
    private static ExprNode FoldBinary(BinOp op, ExprNode a, ExprNode b) {
        if (a is ConstExpr ca && b is ConstExpr cb) {
            return new ConstExpr(op switch {
                BinOp.Add => ca.V + cb.V,
                BinOp.Sub => ca.V - cb.V,
                BinOp.Mul => ca.V * cb.V,
        ...
        if (op == BinOp.Add && b is ConstExpr { V: 0 }) return a;
        if (op == BinOp.Add && a is ConstExpr { V: 0 }) return b;
        if (op == BinOp.Sub && b is ConstExpr { V: 0 }) return a;
        if (op == BinOp.Mul && (a is ConstExpr { V: 0 } || b is ConstExpr { V: 0 })) return new ConstExpr(0);
        if (op == BinOp.Mul && b is ConstExpr { V: 1 }) return a;
        return op == BinOp.Mul && a is ConstExpr { V: 1 } ? b :
    ...
    public static bool IsFullyResolved(ExprNode n) => n switch {
        ...
        SelectExpr s => IsFullyResolved(s.Cond) && IsFullyResolved(s.A) && IsFullyResolved(s.B),
    ...
    public static int CountOpaque(ExprNode n) => n switch {
        ...
        SelectExpr s => CountOpaque(s.Cond) + CountOpaque(s.A) + CountOpaque(s.B),
    ...
    public static int Depth(ExprNode n) => n switch {
        ...
        SelectExpr s => 1 + Math.Max(Depth(s.Cond), Math.Max(Depth(s.A), Depth(s.B))),
    ...
    public static JsonNode ToJson(ExprNode n) => n switch {
        ConstExpr c => new JsonObject { ["op"] = "Const", ["v"] = c.V },
        ...
        SelectExpr s => new JsonObject { ["op"] = "Select", ["cond"] = ToJson(s.Cond), ["a"] = ToJson(s.A), ["b"] = ToJson(s.B) },
...
public sealed record ConstExpr(double V) : ExprNode;
...
public sealed record SelectExpr(ExprNode Cond, ExprNode A, ExprNode B) : ExprNode;
