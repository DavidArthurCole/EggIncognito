...
public sealed class EffectRow
{
    ...
    public double GetDouble(string field) => Convert.ToDouble(_values[field]);
    public int GetInt(string field) => Convert.ToInt32(_values[field]);
    public string GetString(string field) => (string)_values[field];
    public bool GetBool(string field) => (bool)_values[field];

    public bool TryGet(string field, out object value) => _values.TryGetValue(field, out value!);
